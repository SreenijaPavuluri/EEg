"""ShallowConvNet-only LOSO runner. Mirror of 02c but for ShallowConvNet."""
...
OUT = os.path.join(LOGS_DIR, 'shallowconv_results.csv')

from moabb.datasets import PhysionetMI
from moabb.paradigms import MotorImagery
from src.models import ShallowConvNet, TorchClassifierWrapper
...
for i, test in enumerate(subjects):
    if test in done:
        print(f"  ShallowConvNet S{test:03d}: SKIP"); continue
    ...
    t0 = time.time()
    try:
        clf = TorchClassifierWrapper(ShallowConvNet,
              model_kwargs={'n_filters_time':40,'filter_time_length':25,'dropout':0.5},
        ...
        m.update({'subject':test,'method':'ShallowConvNet','n_train':len(X_tr),
                  'n_test':len(X_te),'time_s':round(el,2),'status':'OK'})
        print(f"  ShallowConvNet S{test:03d} [{i+1}/{len(subjects)}]: acc={m['accuracy']:.3f} ece={m['ece']:.3f} [{el:.0f}s]")
    except Exception as e:
        el = time.time()-t0
        m = {'subject':test,'method':'ShallowConvNet','status':f'FAILED:{e}','time_s':round(el,2)}
        print(f"  ShallowConvNet S{test:03d}: FAILED – {e}")
    rows.append(m)
    pd.DataFrame(rows).to_csv(OUT, index=False)

print(f"ShallowConvNet done → {OUT}")
