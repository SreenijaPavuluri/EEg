...
64 channels), we compare six representative methods spanning classical
features, Riemannian geometry, and compact neural architectures. Beyond
accuracy and Cohen's~$\kappa$, we report Expected Calibration Error (ECE)
and Brier score. Our primary finding is that the Riemannian tangent-space
SVM achieves the highest balanced accuracy ($0.663 \pm 0.099$) while
simultaneously yielding the best-calibrated probabilities (ECE\,$=\,0.645 \pm 0.039$,
Brier\,$=\,0.211 \pm 0.035$) among all evaluated methods, and is
statistically significantly better than every alternative
(Wilcoxon $p < 0.001$). These results demonstrate that EEG foundation model
evaluation protocols should routinely include Riemannian baselines and
calibration metrics. All code and results are publicly available.
\end{abstract}
...
\textbf{EEGNet}: Compact depthwise-separable CNN \cite{lawhern2018eegnet},
trained with cross-entropy loss, Adam optimizer, 60 epochs, batch size 64,
early stopping at patience 12.

\textbf{ShallowConvNet}: Shallow temporal-spatial CNN \cite{schirrmeister2017deep},
same training protocol as EEGNet.
...
\input{../results/tables/table_results_final.tex}
...
\subsection{Neural Baselines (EEGNet, ShallowConvNet)}

EEGNet \cite{lawhern2018eegnet} and ShallowConvNet \cite{schirrmeister2017deep}
were evaluated under the same LOSO protocol. Training was done end-to-end with
60 epochs and early stopping. These compact neural architectures represent a
practical upper bound of what a neural approach can achieve under CPU-only
computation constraints without large-scale pretraining.

The neural methods provide a reference point for how much gain (if any) compact
end-to-end learning provides over the Riemannian approaches under CPU-feasible
conditions. See Table~\ref{tab:results} for the complete comparison including
neural methods.
...
directly comparing Riemannian geometry and compact neural methods under
strict LOSO evaluation. Our three main findings are:
(1) Riemannian tangent-space SVM significantly outperforms classical baselines
in balanced accuracy ($0.663 \pm 0.099$, Wilcoxon $p < 0.001$) without any pretraining;
(2) Riemann-TS+SVM simultaneously achieves the best calibration (lowest ECE and
Brier score), showing that accuracy and calibration need not trade off;
